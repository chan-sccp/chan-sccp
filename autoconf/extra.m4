...
AC_DEFUN([CS_ENABLE_OPTIMIZATION], [
	AC_ARG_ENABLE(optimization, [
		AC_HELP_STRING([--enable-optimization],[no detection or tuning flags for cpu version])], [enable_optimization=$enableval],[enable_optimization=no; if test "${REPOS_TYPE}" = "TGZ"; then enable_optimization=yes; fi])
...
AC_DEFUN([CS_ENABLE_STRIP], [
	AC_ARG_ENABLE(strip, 
	  ...
	  ac_cv_enable_strip=$enableval, ac_cv_enable_strip=no)
	strip_binaries="${ac_cv_enable_strip}"
	AC_MSG_RESULT([--enable-strip: ${ac_cv_enable_strip}])
	AC_SUBST([strip_binaries])
])

AC_DEBUG([CS_DISABLE_SECTION_RELOCATION], [
	AC_ARG_ENABLE(section_relocate,
	  AC_HELP_STRING([--disable-section-relocation], [disable compiler section relocation]),
	  ac_cv_use_section_relocation=$enableval, ac_cv_section_relocation=yes)
