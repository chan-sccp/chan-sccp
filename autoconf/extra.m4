...
AC_DEFUN([AST_SET_PBX_AMCONDITIONALS],[
	...
	AM_CONDITIONAL([ASTERISK_VER_GROUP_119], [test x${ASTTERISK_VER_GROUP} = x119])
...
AC_DEFUN([CS_SETUP_DOXYGEN], [
	...
	AS_IF([test "_${ac_cv_use_devdoc}" == "_yes"], [DX_ENV_APPEND([INPUT],[. src src/pbx_impl src/pbx_impl/ast src/pbx_impl/ast116 src/pbx_impl/ast117 src/pbx_impl/ast118 src/pbx_impl/ast119 src/pbx_impl/ast_announce])])
	DX_HTML_FEATURE(ON)
	DX_CHM_FEATURE(OFF)
	DX_CHI_FEATURE(OFF)
	DX_MAN_FEATURE(OFF)
...
AC_DEFUN([CS_ENABLE_EXPERIMENTAL_XML], [
	...
	AS_IF([test "_${ac_cv_experimental_xml}" == "_yes" ], [
		...
		[
			HAVE_LIBXML2=yes
			AC_DEFINE(HAVE_LIBXML2, 1, [Found 'LIBXML2'])
		], [
			HAVE_LIBXML2=no
		])
		...
		[
			HAVE_LIBXSLT=yes
			AC_DEFINE(HAVE_LIBXSLT, 1, [Found 'LIBXSLT'])
		], [
			HAVE_LIBXSLT=no
		])
		...
		[
			HAVE_LIBEXSLT=yes
			AC_DEFINE(HAVE_LIBEXSLT, 1, [Found 'LIBEXSLT'])
		], [
			HAVE_LIBEXSLT=no
		])
