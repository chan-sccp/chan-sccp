...
static int sccp_func_sccpdevice(PBX_CHANNEL_TYPE * chan, NEWCONST char *cmd, char *data, char *output, size_t len)
{
	...
	char *colname = NULL;											// we should make this a finite length
	...
	if (l) {
		...
		while (token != NULL) {
			...
			if (!strcasecmp(token, "id")) {
				...
				pbx_str_t *lbuf = pbx_str_alloca(DEFAULT_PBX_STR_BUFFERSIZE);
				SCCP_EMB_RWLIST_RDLOCK(&l->mailboxes);
				SCCP_EMB_RWLIST_TRAVERSE(&l->mailboxes, mailbox, list) { pbx_str_append(&lbuf, 0, "%s%s", addcomma++ ? "," : "", mailbox->uniqueid); }
