...
//		res = AST_DEVICE_BUSY;
                ...
                /* we should not answer with AST_DEVICE_INUSE */
                //res = AST_DEVICE_NOT_INUSE;
                //sccp_log(1)(VERBOSE_PREFIX_3 "SCCP: Asterisk asked we answer else state\n");
...
//	sccp_log(10)(VERBOSE_PREFIX_3 "%s(%d): >> Got message %s\n", (s->device)?s->device->id:"null", (s->device)?s->device->session->sin.sin_addr.s_addr:0 ,sccpmsg2str(mid));
        /*try to handle nat problem. devices behind nat box can not do anything after ip address change */
...
static int load_module(void) {
	...
	sccp_lineTemplate = ast_malloc(sizeof(sccp_line_t));
	sccp_createTemplateDevice();
	sccp_createTemplateLine();
        ...
        GLOB(pushPlacedCalls)  = TRUE;
        GLOB(pushReceivedCalls)  = TRUE;
...
void sccp_createTemplateDevice(){
	...
	sccp_deviceTemplate->linesCount = 0;
        sccp_deviceTemplate->accessoryused = 0;
        sccp_deviceTemplate->realtime = FALSE;
        sccp_deviceTemplate->accessorystatus = 0;
        sccp_deviceTemplate->tz_offset = 0;
        sccp_deviceTemplate->defaultLineInstance = 0;
	sccp_deviceTemplate->transfer = 1;
	sccp_deviceTemplate->state = SCCP_DEVICESTATE_ONHOOK;
	sccp_deviceTemplate->privacyFeature.enabled = TRUE;
        sccp_deviceTemplate->monitorFeature.enabled = TRUE;
	sccp_deviceTemplate->postregistration_thread = AST_PTHREADT_STOP;
#ifdef CS_SCCP_PICKUP
        sccp_deviceTemplate->pickupexten = 0;
        sccp_deviceTemplate->pickupcontext = NULL;
#endif

#ifdef CS_SCCP_PARK
        sccp_deviceTemplate->park = 1;
#else
        sccp_deviceTemplate->park = 0;
#endif
        /* reset statistic */
        sccp_deviceTemplate->configurationStatistic.numberOfLines=0;
        sccp_deviceTemplate->configurationStatistic.numberOfSpeeddials=0;
        sccp_deviceTemplate->configurationStatistic.numberOfFeatures=0;
	sccp_deviceTemplate->dndmode = SCCP_DNDMODE_REJECT;
	sccp_deviceTemplate->capability = AST_FORMAT_ALAW|AST_FORMAT_ULAW|AST_FORMAT_G729A;
        sccp_deviceTemplate->directrtp = FALSE;
	sccp_deviceTemplate->nat = FALSE;
	sccp_deviceTemplate->mwilamp = SKINNY_LAMP_ON;
	sccp_deviceTemplate->earlyrtp = 0;
	sccp_deviceTemplate->overlapFeature.enabled = 0;
	sccp_deviceTemplate->trustphoneip = FALSE;
	sccp_deviceTemplate->keepalive = 60;
        
	/*
	sccp_deviceTemplate->codecs = GLOB(global_codecs);
        sccp_deviceTemplate->trustphoneip = GLOB(trustphoneip);
        sccp_deviceTemplate->earlyrtp = GLOB(earlyrtp);
        sccp_deviceTemplate->mwioncall = GLOB(mwioncall);
        sccp_deviceTemplate->cfwdall = GLOB(cfwdall);
        sccp_deviceTemplate->cfwdbusy = GLOB(cfwdbusy);
        sccp_deviceTemplate->cfwdnoanswer = GLOB(cfwdnoanswer);
	*/
}

void sccp_createTemplateLine(){
	sccp_lineTemplate = ast_malloc(sizeof(sccp_line_t));
	memset(&sccp_lineTemplate, 0, sizeof(sccp_line_t));
	

        sccp_lineTemplate->incominglimit = 3; /* default value */
        sccp_lineTemplate->echocancel = TRUE; /* default value */

        sccp_lineTemplate->silencesuppression = FALSE; /* default value */
        sccp_lineTemplate->rtptos = (184 & 0xff); /* default value */

        sccp_lineTemplate->transfer = TRUE; /* default value. on if the device transfer is on*/
        sccp_lineTemplate->secondary_dialtone_tone = SKINNY_TONE_OUTSIDEDIALTONE;
        sccp_lineTemplate->dndmode = SCCP_DNDMODE_OFF;
        sccp_copy_string(sccp_lineTemplate->context, "sccp", sizeof(sccp_lineTemplate->context));
        sccp_copy_string(sccp_lineTemplate->language, "en", sizeof(sccp_lineTemplate->language));
        sccp_lineTemplate->amaflags = ast_cdr_amaflags2int("documentation");
	
	//TODO what can the default values be
// 	sccp_copy_string(sccp_lineTemplate->accountcode, GLOB(accountcode), sizeof(sccp_lineTemplate->accountcode));
//         sccp_copy_string(sccp_lineTemplate->musicclass, GLOB(musicclass), sizeof(sccp_lineTemplate->musicclass));
// 	sccp_lineTemplate->callgroup = GLOB(callgroup);

#ifdef CS_SCCP_PICKUP
        //sccp_lineTemplate->pickupgroup = GLOB(pickupgroup);
#endif
