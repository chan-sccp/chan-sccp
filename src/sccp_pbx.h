...
SCCP_API uint8_t SCCP_CALL sccp_pbx_channel_allocate(sccp_channel_t * c, const void *ids, const PBX_CHANNEL_TYPE * parentChannel);
...
SCCP_API void * SCCP_CALL sccp_pbx_softswitch(sccp_channel_t * c);
...
SCCP_API int SCCP_CALL sccp_pbx_hangup(sccp_channel_t * c);
SCCP_API int SCCP_CALL sccp_pbx_call(sccp_channel_t * c, char *dest, int timeout);
SCCP_API int SCCP_CALL sccp_pbx_answer(sccp_channel_t * c);
__END_C_EXTERN__
