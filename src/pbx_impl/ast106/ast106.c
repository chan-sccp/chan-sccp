...
//static PBX_CHANNEL_TYPE *sccp_astwrap_request(const char *type, format_t format, const PBX_CHANNEL_TYPE * requestor, void *data, int *cause);
static PBX_CHANNEL_TYPE *sccp_astwrap_request(const char *type, int format, void *data, int *cause);
...
static int sccp_astwrap_recvdigit_begin(PBX_CHANNEL_TYPE * ast, char digit);
static int sccp_astwrap_recvdigit_end(PBX_CHANNEL_TYPE * ast, char digit, unsigned int duration);
...
boolean_t sccp_astwrap_getForwarderPeerChannel(const sccp_channel_t * channel, PBX_CHANNEL_TYPE ** pbx_channel);
...
static struct ast_channel_tech sccp_tech = {
	...
	send_digit_begin:	sccp_astwrap_recvdigit_begin,
	send_digit_end:		sccp_astwrap_recvdigit_end,
...
static int sccp_astwrap_indicate(PBX_CHANNEL_TYPE * ast, int ind, const void *data, size_t datalen)
{
	...
	if (!d) {
		...
		if (c->parentChannel) {
			...
			if (sccp_astwrap_getForwarderPeerChannel(c->parentChannel, &bridgePeer)) {
				sccp_log((DEBUGCAT_PBX | DEBUGCAT_INDICATE)) (VERBOSE_PREFIX_3 "SCCP: forwarding indication %d to caller %s\n", ind, bridgePeer->name);
				res = sccp_astwrap_indicate(bridgePeer, ind, data, datalen);
			...
			ast_log(LOG_WARNING, "SCCP: (indicate) Could not find device to handle indication, giving up.\n");
			res = -1;
		}
		return res;
	}
	...
	switch (ind) {
		...
		case AST_CONTROL_HOLD:
			...
			if (c->rtp.video.instance && d && sccp_device_isVideoSupported(d) && sccp_channel_getVideoMode(c) != SCCP_VIDEO_MODE_OFF) {
				ast_rtp_new_source(c->rtp.video.instance);
...
static int sccp_astwrap_sendDigits(const sccp_channel_t * channel, const char *digits)
{
...
static int sccp_astwrap_sendDigit(const sccp_channel_t * channel, const char digit)
{
	char digits[3] = "\0\0";
	...
	return sccp_astwrap_sendDigits(channel, digits);
}

static void sccp_astwrap_setCalleridPresentation(PBX_CHANNEL_TYPE * pbxChannel, sccp_callerid_presentation_t presentation)
{
...
static int sccp_astwrap_setNativeAudioFormats(const sccp_channel_t * channel, skinny_codec_t codec[])
{

	format_t new_nativeformats = 0;
	int i;
	if (!channel || !channel->owner || !channel->owner->nativeformats) {
		pbx_log(LOG_ERROR, "SCCP: (asterisk111_setNativeAudioFormats) no channel provided!\n");
		return 0;
	}
...
static int sccp_astwrap_setNativeVideoFormats(const sccp_channel_t * channel, skinny_codec_t codec[])
{
	return 1;
}
 ...
 */
static void *sccp_astwrap_park_thread(void *data)
{
...
static sccp_parkresult_t sccp_astwrap_park(const sccp_channel_t * hostChannel)
{
	...
	if ((arg->device = sccp_channel_getDevice(hostChannel))) {
		if (!ast_pthread_create_detached_background(&th, NULL, sccp_astwrap_park_thread, arg)) {
...
static int ast_do_pickup(PBX_CHANNEL_TYPE * chan, PBX_CHANNEL_TYPE * target)
{
	...
	if (queue_control(chan, AST_CONTROL_ANSWER)) {
...
static int sccp_astwrap_call(PBX_CHANNEL_TYPE * ast, char *dest, int timeout)
{
	...
	sccp_astwrap_callerid_name(c->owner, &cid_name);
	sccp_astwrap_callerid_number(c->owner, &cid_number);
...
static int sccp_astwrap_fixup(PBX_CHANNEL_TYPE * oldchan, PBX_CHANNEL_TYPE * newchan)
{
	...
	if (!c) {
		...
		if (c->owner != oldchan) {
			...
			// sccp_astwrap_asterisk111_update_rtp_peer(newchan, NULL, NULL, 0, 0, 0);
...
static enum ast_rtp_get_result sccp_astwrap_get_rtp_info(PBX_CHANNEL_TYPE * ast, PBX_RTP_TYPE ** rtp)
{
	//AUTO_RELEASE(sccp_channel_t, c , get_sccp_channel_from_pbx_channel(ast));				// not following the refcount rules... channel is already retained
	sccp_channel_t *c = NULL;
	...
	*rtp = audioRTP->instance;
	...
	// sccp_log((DEBUGCAT_RTP | DEBUGCAT_HIGH)) (VERBOSE_PREFIX_3 "SCCP: (get_rtp_info) remote address:'%s:%d'\n", ast_sockaddr_stringify_host(&ast_sockaddr_tmp), ast_sockaddr_port(&ast_sockaddr_tmp));
...
static enum ast_rtp_get_result sccp_astwrap_get_vrtp_info(PBX_CHANNEL_TYPE * ast, PBX_RTP_TYPE ** rtp)
{
	//AUTO_RELEASE(sccp_channel_t, c , get_sccp_channel_from_pbx_channel(ast));				// not following the refcount rules... channel is already retained
	sccp_channel_t *c = NULL;
	sccp_rtp_info_t rtpInfo = SCCP_RTP_INFO_NORTP;
...
static int sccp_astwrap_update_rtp_peer(PBX_CHANNEL_TYPE * ast, PBX_RTP_TYPE * rtp, PBX_RTP_TYPE * vrtp, PBX_RTP_TYPE * trtp, int codecs, int nat_active)
{
...
static int sccp_astwrap_getCodec(PBX_CHANNEL_TYPE * ast)
{
...
static uint sccp_astwrap_get_dtmf_payload_code(constChannelPtr c)
{
	int rtp_code = 0;
...
static boolean_t sccp_astwrap_getChannelByName(const char *name, PBX_CHANNEL_TYPE ** pbx_channel)
{
	...
	while ((ast_channel = ast_channel_walk_locked(ast_channel)) != NULL) {
		if (strlen(ast_channel->name) == strlen(name) && !strncmp(ast_channel->name, name, strlen(ast_channel->name))) {
			*pbx_channel = pbx_channel_ref(ast_channel);
...
static int sccp_astwrap_setPhoneRTPAddress(const struct sccp_rtp *rtp, const struct sockaddr_storage *new_peer, int nat_active)
{
	...
	// sccp_log((DEBUGCAT_RTP | DEBUGCAT_HIGH)) (VERBOSE_PREFIX_3 "SCCP: (setPhoneRTPAddress) Set rtp_instance NAT Property to %s\n", S_COR(nat_active, "True", "False"));
	if (nat_active) {
...
{
	sccp_log(DEBUGCAT_CODEC) (VERBOSE_PREFIX_3 "sccp_astwrap_setRedirectedParty Not Implemented\n");
...
{
	sccp_log(DEBUGCAT_CODEC) (VERBOSE_PREFIX_3 "sccp_astwrap_updateConnectedLine Not Implemented\n");
...
static void sccp_astwrap__set_pbxchannel_linkedid(PBX_CHANNEL_TYPE * pbx_channel, const char *new_linkedid)
{
	if (pbx_channel) {
...
static const char *sccp_astwrap__get_channel_##_field(const sccp_channel_t * channel)	 		\
{														\
...
static void sccp_astwrap__set_channel_##_field(const sccp_channel_t * channel, const char * (_field))	\
{ 														\
        if (channel && channel->owner) {											\
...
static const char *sccp_astwrap__get_channel_linkedid(const sccp_channel_t * channel)
{
...
static void sccp_astwrap__set_channel_linkedid(const sccp_channel_t * channel, const char *new_linkedid)
{
	if (channel && channel->owner) {
		sccp_astwrap__set_pbxchannel_linkedid(channel->owner, new_linkedid);
	}
}

static void sccp_astwrap__set_channel_name(const sccp_channel_t * channel, const char *new_name)
{
	if (channel && channel->owner) {
...
static void sccp_astwrap__set_channel_call_forward(const sccp_channel_t * channel, const char *new_call_forward)
{
	if (channel && channel->owner) {
...
static enum ast_channel_state sccp_astwrap__get_channel_state(const sccp_channel_t * channel)
{
	if (channel && channel->owner) {
...
static const struct ast_pbx *sccp_astwrap__get_channel_pbx(const sccp_channel_t * channel)
{
	if (channel && channel->owner) {
...
{
	PBX_CHANNEL_TYPE *remotePeer = ast_channel_search_locked(pbx_find_channel_by_linkedid, (void *) sccp_astwrap__get_channel_linkedid(channel));
...
static struct ast_rtp_protocol sccp_rtp = {
	...
	.set_rtp_peer = sccp_astwrap_update_rtp_peer,
...
static int sccp_astwrap_message_send(const struct ast_msg *msg, const char *to, const char *from)
{
	...
	SCCP_EMB_RWLIST_LOCK(&line->devices);
...
static int sccp_astwrap_register_application(const char *app_name, int (*execute)(struct ast_channel *, void *))
{
	return ast_register_application2(app_name, execute, NULL, NULL, ast_module_info->self);
}

static int sccp_astwrap_unregister_application(const char *app_name)
{
	return ast_unregister_application(app_name);
}

static int sccp_astwrap_register_function(struct pbx_custom_function *custom_function)
{
	return __ast_custom_function_register(custom_function, ast_module_info->self);
}

static int sccp_astwrap_unregister_function(struct pbx_custom_function *custom_function)
{
	return ast_custom_function_unregister(custom_function);
}

static boolean_t sccp_astwrap__setLanguage(PBX_CHANNEL_TYPE * pbxChannel, const char *newLanguage)
{
...
const PbxInterface iPbx = {
	...
	setPBXChannelLinkedId:		sccp_astwrap__set_pbxchannel_linkedid,
	/** get channel by name */
	getChannelByName:		sccp_astwrap_getChannelByName,
	getRemoteChannel:		sccp_astwrap_getRemoteChannel,
	getChannelByCallback:		NULL,

	getChannelLinkedId:		sccp_astwrap__get_channel_linkedid,
	setChannelLinkedId:		sccp_astwrap__set_channel_linkedid,
	getChannelName:			sccp_astwrap__get_channel_name,
	setChannelName:			sccp_astwrap__set_channel_name,
	getChannelUniqueID:		sccp_astwrap__get_channel_uniqueid,
	getChannelExten:		sccp_astwrap__get_channel_exten,
	setChannelExten:		sccp_astwrap__set_channel_exten,
	getChannelContext:		sccp_astwrap__get_channel_context,
	setChannelContext:		sccp_astwrap__set_channel_context,
	getChannelMacroExten:		sccp_astwrap__get_channel_macroexten,
	setChannelMacroExten:		sccp_astwrap__set_channel_macroexten,
	getChannelMacroContext:		sccp_astwrap__get_channel_macrocontext,
	setChannelMacroContext:		sccp_astwrap__set_channel_macrocontext,
	getChannelCallForward:		sccp_astwrap__get_channel_call_forward,
	setChannelCallForward:		sccp_astwrap__set_channel_call_forward,

	getChannelAppl:			sccp_astwrap__get_channel_appl,
	getChannelState:		sccp_astwrap__get_channel_state,
	getChannelPbx:			sccp_astwrap__get_channel_pbx,
	...
	send_digit:			sccp_astwrap_sendDigit,
	send_digits:			sccp_astwrap_sendDigits,
	...
	set_language:			sccp_astwrap__setLanguage,
	...
	register_application:		sccp_astwrap_register_application,
	unregister_application:		sccp_astwrap_unregister_application,
	register_function:		sccp_astwrap_register_function,
	unregister_function:		sccp_astwrap_unregister_function,

	get_dtmf_payload_code:		sccp_astwrap_get_dtmf_payload_code,
	/* *INDENT-ON* */
};
...
/* sccp_astwrap_findPickupChannelByExtenLocked helper functions */
...
/* end sccp_astwrap_findPickupChannelByExtenLocked helper functions */
