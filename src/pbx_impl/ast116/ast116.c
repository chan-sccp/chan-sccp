...
static PBX_CHANNEL_TYPE *sccp_wrapper_asterisk116_request(const char *type, struct ast_format_cap *cap, const struct ast_assigned_ids *assignedids, const PBX_CHANNEL_TYPE * requestor, const char *dest, int *cause);
static int sccp_wrapper_asterisk116_call(PBX_CHANNEL_TYPE * ast, const char *dest, int timeout);
static int sccp_wrapper_asterisk116_answer(PBX_CHANNEL_TYPE * chan);
static PBX_FRAME_TYPE *sccp_wrapper_asterisk116_rtp_read(PBX_CHANNEL_TYPE * ast);
static int sccp_wrapper_asterisk116_rtp_write(PBX_CHANNEL_TYPE * ast, PBX_FRAME_TYPE * frame);
static int sccp_wrapper_asterisk116_indicate(PBX_CHANNEL_TYPE * ast, int ind, const void *data, size_t datalen);
static int sccp_wrapper_asterisk116_fixup(PBX_CHANNEL_TYPE * oldchan, PBX_CHANNEL_TYPE * newchan);
static void sccp_wrapper_asterisk116_setDialedNumber(constChannelPtr channel, const char *number);

//#ifdef CS_AST_RTP_INSTANCE_BRIDGE
//static enum ast_bridge_result sccp_wrapper_asterisk116_rtpBridge(PBX_CHANNEL_TYPE * c0, PBX_CHANNEL_TYPE * c1, int flags, PBX_FRAME_TYPE ** fo, PBX_CHANNEL_TYPE ** rc, int timeoutms);
//#endif
...
int sccp_wrapper_asterisk116_hangup(PBX_CHANNEL_TYPE * ast_channel);
...
static int sccp_wrapper_asterisk116_devicestate(const char *data);
static int sccp_wrapper_asterisk116_setNativeAudioFormats(constChannelPtr channel, skinny_codec_t codecs[], int length);
static boolean_t sccp_wrapper_asterisk116_setWriteFormat(constChannelPtr channel, skinny_codec_t codec);
static boolean_t sccp_wrapper_asterisk116_setReadFormat(constChannelPtr channel, skinny_codec_t codec);
...
static struct ast_format *sccp_asterisk116_skinny2ast_format(skinny_codec_t skinnycodec)
{
	switch (skinnycodec) {
...
static void pbx_format_cap_append_skinny(struct ast_format_cap *caps, skinny_codec_t codecs[SKINNY_MAX_CAPABILITIES]) {
	...
	for (n=0; n<SKINNY_MAX_CAPABILITIES; n++) {
		...
		struct ast_format *format = sccp_asterisk116_skinny2ast_format(codecs[n]);
...
//      capabilities:		AST_FORMAT_ALAW | AST_FORMAT_ULAW | AST_FORMAT_SLINEAR16 | AST_FORMAT_GSM | AST_FORMAT_G723_1 | AST_FORMAT_G729A | AST_FORMAT_H264 | AST_FORMAT_H263_PLUS,
	...
	requester:		sccp_wrapper_asterisk116_request,
	devicestate:		sccp_wrapper_asterisk116_devicestate,
	send_digit_begin:	sccp_wrapper_recvdigit_begin,
	send_digit_end:		sccp_wrapper_recvdigit_end,
	call:			sccp_wrapper_asterisk116_call,
	hangup:			sccp_wrapper_asterisk116_hangup,
	answer:			sccp_wrapper_asterisk116_answer,
	read:			sccp_wrapper_asterisk116_rtp_read,
	write:			sccp_wrapper_asterisk116_rtp_write,
	send_text:		sccp_pbx_sendtext,
	send_image:		NULL,
	send_html:		sccp_pbx_sendHTML,
	exception:		NULL,
//	bridge:			sccp_wrapper_asterisk116_rtpBridge,
        bridge:			ast_rtp_instance_bridge,
	early_bridge:		NULL,
	indicate:		sccp_wrapper_asterisk116_indicate,
	fixup:			sccp_wrapper_asterisk116_fixup,
	setoption:		NULL,
	queryoption:		NULL,
	transfer:		NULL,
	write_video:		sccp_wrapper_asterisk116_rtp_write,
	write_text:		NULL,
...
//	.bridge 		= sccp_wrapper_asterisk116_rtpBridge,
	...
	//.exception            =                                              // new >1.6.0
	//.setoption            = sccp_wrapper_asterisk116_setOption,
		...
		case AST_CONTROL_RINGING:
			if (SKINNY_CALLTYPE_OUTBOUND == c->calltype) {
				...
				if (d->earlyrtp == SCCP_EARLYRTP_IMMEDIATE) {
					...
					sccp_wrapper_asterisk116_setDialedNumber(c, c->dialedNumber);
				}
				...
				for (; (remotePeer = ast_channel_iterator_next(iterator)); ast_channel_unref(remotePeer)) {
					if (pbx_find_channel_by_linkid(remotePeer, (void *) ast_channel_linkedid(ast))) {
						...
						if (remoteSccpChannel) {
							...
							sccp_asterisk116_getSkinnyFormatMultiple(ast_channel_nativeformats(remotePeer), c->remoteCapabilities.audio, ARRAY_LEN(c->remoteCapabilities.audio));
						}
...
static void sccp_wrapper_asterisk116_setDialedNumber(const sccp_channel_t *channel, const char *number)
{
...
static void sccp_wrapper_asterisk116_setCalleridPresentation(PBX_CHANNEL_TYPE *pbx_channel, sccp_callerid_presentation_t presentation)
{
...
static void sccp_wrapper_asterisk116_removeTimingFD(PBX_CHANNEL_TYPE *ast)
{
	if (ast) {
...
static void sccp_wrapper_asterisk116_setOwner(sccp_channel_t * channel, PBX_CHANNEL_TYPE * pbx_channel)
{
...
static void __sccp_asterisk116_updateConnectedLine(PBX_CHANNEL_TYPE *pbx_channel, const char *number, const char *name, uint8_t reason)
{
	if (!pbx_channel) {
		return;
	}
...
static boolean_t sccp_wrapper_asterisk116_allocPBXChannel(sccp_channel_t * channel, const void *ids, const PBX_CHANNEL_TYPE * pbxSrcChannel, PBX_CHANNEL_TYPE ** _pbxDstChannel)
{
	...
	sccp_wrapper_asterisk116_setOwner(channel, pbxDstChannel);
 ...
 */
static boolean_t sccp_wrapper_asterisk116_masqueradeHelper(PBX_CHANNEL_TYPE * pbxChannel, PBX_CHANNEL_TYPE * pbxTmpChannel)
{
	boolean_t res = FALSE;
...
static boolean_t sccp_wrapper_asterisk116_allocTempPBXChannel(PBX_CHANNEL_TYPE * pbxSrcChannel, PBX_CHANNEL_TYPE ** _pbxDstChannel)
{
	PBX_CHANNEL_TYPE *pbxDstChannel = NULL;
...
static PBX_CHANNEL_TYPE *sccp_wrapper_asterisk116_requestAnnouncementChannel(pbx_format_type format_type, const PBX_CHANNEL_TYPE * requestor, void *data)
{
 ...
 *
 */
static sccp_parkresult_t sccp_wrapper_asterisk116_park(constChannelPtr hostChannel)
{
...
static boolean_t sccp_wrapper_asterisk116_getFeatureExtension(constChannelPtr channel, const char *featureName, char extension[SCCP_MAX_EXTENSION])
{
	int feat_res = ast_get_feature(channel->owner, featureName, extension, SCCP_MAX_EXTENSION);
	return feat_res ? FALSE : TRUE;
}

static boolean_t sccp_wrapper_asterisk116_getPickupExtension(constChannelPtr channel, char extension[SCCP_MAX_EXTENSION])
{
	boolean_t res = FALSE;
...
static uint8_t sccp_wrapper_asterisk116_get_payloadType(const struct sccp_rtp *rtp, skinny_codec_t codec)
{
	struct ast_format *astCodec = sccp_asterisk116_skinny2ast_format(codec);
...
static int sccp_wrapper_asterisk116_get_sampleRate(skinny_codec_t codec)
{
	struct ast_format *astCodec = sccp_asterisk116_skinny2ast_format(codec);
	if (astCodec != ast_format_none) {
...
static sccp_extension_status_t sccp_wrapper_asterisk116_extensionStatus(constChannelPtr channel)
{
	PBX_CHANNEL_TYPE *pbx_channel = channel->owner;
...
static PBX_CHANNEL_TYPE *sccp_wrapper_asterisk116_request(const char *type, struct ast_format_cap *cap, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *dest, int *cause)
{
...
static int sccp_wrapper_asterisk116_fixup(PBX_CHANNEL_TYPE * oldchan, PBX_CHANNEL_TYPE * newchan)
{
	...
	if (c) {
		if (c->owner != oldchan) {
			...
			sccp_wrapper_asterisk116_setOwner(c, newchan);
			//! \todo force update of rtp peer for directrtp
			// sccp_wrapper_asterisk116_update_rtp_peer(newchan, NULL, NULL, 0, 0, 0);
...
static enum ast_rtp_glue_result sccp_wrapper_asterisk116_get_rtp_info(PBX_CHANNEL_TYPE * ast, PBX_RTP_TYPE ** rtp)
{
	...
	if (!(c = CS_AST_CHANNEL_PVT(ast))) {
		sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "SCCP: (asterisk116_get_rtp_info) NO PVT\n");
		return AST_RTP_GLUE_RESULT_FORBID;
	}

	if (pbx_channel_state(ast) != AST_STATE_UP) {
		sccp_log((DEBUGCAT_CHANNEL | DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_rtp_info) Asterisk requested EarlyRTP peer for channel %s\n", c->currentDeviceId, pbx_channel_name(ast));
	} else {
		sccp_log((DEBUGCAT_CHANNEL | DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_rtp_info) Asterisk requested RTP peer for channel %s\n", c->currentDeviceId, pbx_channel_name(ast));
	}
	...
	*rtp = audioRTP->instance;
	...
	// sccp_log((DEBUGCAT_RTP | DEBUGCAT_HIGH)) (VERBOSE_PREFIX_3 "%s: (asterisk116_get_rtp_info) remote address:'%s:%d'\n", c->currentDeviceId, ast_sockaddr_stringify_host(&ast_sockaddr_tmp), ast_sockaddr_port(&ast_sockaddr_tmp));

	ao2_ref(*rtp, +1);
	if (ast_test_flag(GLOB(global_jbconf), AST_JB_FORCED)) {
		sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_rtp_info) JitterBuffer is Forced. AST_RTP_GET_FAILED\n", c->currentDeviceId);
		return AST_RTP_GLUE_RESULT_LOCAL;
	}

	if (!(rtpInfo & SCCP_RTP_INFO_ALLOW_DIRECTRTP)) {
		sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_rtp_info) Direct RTP disabled ->  Using AST_RTP_TRY_PARTIAL for channel %s\n", c->currentDeviceId, pbx_channel_name(ast));
		return AST_RTP_GLUE_RESULT_LOCAL;
	}

	sccp_log((DEBUGCAT_RTP | DEBUGCAT_HIGH)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_rtp_info) Channel %s Returning res: %s\n", c->currentDeviceId, pbx_channel_name(ast), ((res == 2) ? "indirect-rtp" : ((res == 1) ? "direct-rtp" : "forbid")));
	return res;
}

static enum ast_rtp_glue_result sccp_wrapper_asterisk116_get_vrtp_info(PBX_CHANNEL_TYPE * ast, PBX_RTP_TYPE ** rtp)
{
	//AUTO_RELEASE(sccp_channel_t, c , get_sccp_channel_from_pbx_channel(ast));				// not following the refcount rules... channel is already retained
	sccp_channel_t *c = NULL;
	sccp_rtp_info_t rtpInfo = SCCP_RTP_INFO_NORTP;
	...
	if (!(c = CS_AST_CHANNEL_PVT(ast))) {
		sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "SCCP: (asterisk116_get_vrtp_info) NO PVT\n");
		return AST_RTP_GLUE_RESULT_FORBID;
	}

	if (pbx_channel_state(ast) != AST_STATE_UP) {
		sccp_log((DEBUGCAT_CHANNEL | DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_vrtp_info) Asterisk requested EarlyRTP peer for channel %s\n", c->currentDeviceId, pbx_channel_name(ast));
	} else {
		sccp_log((DEBUGCAT_CHANNEL | DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_vrtp_info) Asterisk requested RTP peer for channel %s\n", c->currentDeviceId, pbx_channel_name(ast));
	}
...
#endif
	if (ast_test_flag(GLOB(global_jbconf), AST_JB_FORCED)) {
		sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_vrtp_info) JitterBuffer is Forced. AST_RTP_GET_FAILED\n", c->currentDeviceId);
		return AST_RTP_GLUE_RESULT_FORBID;
	}

	if (!(rtpInfo & SCCP_RTP_INFO_ALLOW_DIRECTRTP)) {
		sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_vrtp_info) Direct RTP disabled ->  Using AST_RTP_TRY_PARTIAL for channel %s\n", c->currentDeviceId, pbx_channel_name(ast));
		return AST_RTP_GLUE_RESULT_LOCAL;
	}

	sccp_log((DEBUGCAT_RTP | DEBUGCAT_HIGH)) (VERBOSE_PREFIX_1 "%s: (asterisk116_get_vrtp_info) Channel %s Returning res: %s\n", c->currentDeviceId, pbx_channel_name(ast), ((res == 2) ? "indirect-rtp" : ((res == 1) ? "direct-rtp" : "forbid")));
	return res;
}

static int sccp_wrapper_asterisk116_update_rtp_peer(PBX_CHANNEL_TYPE * ast, PBX_RTP_TYPE * rtp, PBX_RTP_TYPE * vrtp, PBX_RTP_TYPE * trtp, const struct ast_format_cap *codecs, int nat_active)
{
	//AUTO_RELEASE(sccp_channel_t, c , get_sccp_channel_from_pbx_channel(ast));				// not following the refcount rules... channel is already retained
	sccp_channel_t *c = NULL;
	int result = 0;
	do {
		...
		if (!(c = CS_AST_CHANNEL_PVT(ast))) {
			sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "SCCP: (asterisk116_update_rtp_peer) NO PVT\n");
			result = -1;
			break;
		}

		if (!codecs) {
			sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_update_rtp_peer) NO Codecs\n", c->currentDeviceId);
			result = -1;
			break;
		}
		sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_2 "%s: (asterisk116_update_rtp_peer) stage: %s, remote codecs capabilty: %s (%lu), nat_active: %d\n", c->currentDeviceId, S_COR(AST_STATE_UP == pbx_channel_state(ast), "RTP", "EarlyRTP"), ast_format_cap_get_names((struct ast_format_cap *) codecs, &codec_buf), (long unsigned int) codecs, nat_active);

		if (!c->line) {
			sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_update_rtp_peer) NO LINE\n", c->currentDeviceId);
			result = -1;
			break;
		}
		if (c->state == SCCP_CHANNELSTATE_HOLD) {
			sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_update_rtp_peer) On Hold -> No Update\n", c->currentDeviceId);
			result = 0;
			break;
		}
		AUTO_RELEASE(sccp_device_t, d , sccp_channel_getDevice(c));
		if (!d) {
			sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_update_rtp_peer) NO DEVICE\n", c->currentDeviceId);
			result = -1;
			break;
		}

		if (!rtp && !vrtp && !trtp) {
			sccp_log((DEBUGCAT_RTP)) (VERBOSE_PREFIX_1 "%s: (asterisk116_update_rtp_peer) RTP not ready\n", c->currentDeviceId);
			result = 0;
			break;
		}
...
*/
	...
	sccp_log(DEBUGCAT_RTP) (VERBOSE_PREFIX_3 "%s: (asterisk116_update_rtp_peer) Result: %d\n", c->currentDeviceId, result);
	return result;
}

static void sccp_wrapper_asterisk116_getCodec(PBX_CHANNEL_TYPE * ast, struct ast_format_cap *result)
{
	uint8_t i;
	...
	for (i = 0; i < ARRAY_LEN(channel->preferences.audio); i++) {
		struct ast_format *ast_format = sccp_asterisk116_skinny2ast_format(channel->preferences.audio[i]);
	...
	for (i = 0; i < ARRAY_LEN(channel->preferences.video); i++) {
		struct ast_format *ast_format = sccp_asterisk116_skinny2ast_format(channel->preferences.video[i]);
		if (ast_format != ast_format_none) {
			unsigned int framing = ast_format_get_default_ms(ast_format);
			ast_format_cap_append(result, ast_format, framing);
		}
 ...
 * \return parse result
 */
static int sccp_wrapper_asterisk116_callerid_name(PBX_CHANNEL_TYPE *pbx_chan, char **cid_name)
{
 ...
 */
static int sccp_wrapper_asterisk116_callerid_number(PBX_CHANNEL_TYPE *pbx_chan, char **cid_number)
{
 ...
 * \return char * with the caller number
 */
static int sccp_wrapper_asterisk116_callerid_ton(PBX_CHANNEL_TYPE *pbx_chan, int *cid_ton)
{
 ...
 * \return char * with the caller number
 */
static int sccp_wrapper_asterisk116_callerid_ani(PBX_CHANNEL_TYPE *pbx_chan, char **cid_ani)
{
 ...
 * \return char * with the caller number
 */
static int sccp_wrapper_asterisk116_callerid_subaddr(PBX_CHANNEL_TYPE *pbx_chan, char **cid_subaddr)
{
 ...
 * \return char * with the caller number
 */
static int sccp_wrapper_asterisk116_callerid_dnid(PBX_CHANNEL_TYPE *pbx_chan, char **cid_dnid)
{
 ...
 * \return char * with the caller number
 */
static int sccp_wrapper_asterisk116_callerid_rdnis(PBX_CHANNEL_TYPE *pbx_chan, char **cid_rdnis)
{
 ...
 * \return char * with the caller number
 */
static int sccp_wrapper_asterisk116_callerid_presentation(PBX_CHANNEL_TYPE *pbx_chan)
{
...
static boolean_t sccp_wrapper_asterisk116_createRtpInstance(constDevicePtr d, constChannelPtr c, sccp_rtp_t *rtp)
{
...
static boolean_t sccp_wrapper_asterisk116_destroyRTP(PBX_RTP_TYPE * rtp)
{
	int res;
...
static boolean_t sccp_wrapper_asterisk116_checkHangup(constChannelPtr channel)
{
	int res;
...
static boolean_t sccp_wrapper_asterisk116_rtpGetPeer(PBX_RTP_TYPE * rtp, struct sockaddr_storage *address)
{
...
static boolean_t sccp_wrapper_asterisk116_rtpGetUs(PBX_RTP_TYPE * rtp, struct sockaddr_storage *address)
{
	struct ast_sockaddr addr;
...
static boolean_t sccp_wrapper_asterisk116_getChannelByName(const char *name, PBX_CHANNEL_TYPE ** pbx_channel)
{
...
//static int sccp_wrapper_asterisk116_rtp_set_peer(const struct sccp_rtp *rtp, const struct sockaddr_storage *new_peer, int nat_active)
static int sccp_wrapper_asterisk116_setPhoneRTPAddress(const struct sccp_rtp *rtp, const struct sockaddr_storage *new_peer, int nat_active)
{
	...
	sccp_log((DEBUGCAT_RTP | DEBUGCAT_HIGH)) (VERBOSE_PREFIX_3 "SCCP: (asterisk116_setPhoneRTPAddress) Update PBX to send RTP/UDP media to '%s' (new remote) (NAT: %s)\n", ast_sockaddr_stringify(&ast_sockaddr_dest), S_COR(nat_active, "yes", "no"));
	if (nat_active) {
...
static int sccp_wrapper_asterisk116_setNativeAudioFormats(constChannelPtr channel, skinny_codec_t codecs[], int length)
{
	if (!channel || !channel->owner || !ast_channel_nativeformats(channel->owner)) {
		pbx_log(LOG_ERROR, "SCCP: (sccp_wrapper_asterisk116_setNativeAudioFormats) no channel provided!\n");
		return 0;
	}
	struct ast_format_cap *caps = ast_format_cap_alloc(AST_FORMAT_CAP_FLAG_DEFAULT);
	if (!caps) {
	...
	//struct ast_str *codec_buf = ast_str_alloca(AST_FORMAT_CAP_NAMES_LEN);
	//sccp_log((DEBUGCAT_RTP | DEBUGCAT_CODEC)) (VERBOSE_PREFIX_3 "%s: (asterisk116_setNativeAudioFormats) %s\n", channel->designator, ast_format_cap_get_names(ast_channel_nativeformats(channel->owner), &codec_buf));
	return 1;
}

static int sccp_wrapper_asterisk116_setNativeVideoFormats(constChannelPtr channel, skinny_codec_t codec)
{
	if (!channel || !channel->owner || !ast_channel_nativeformats(channel->owner)) {
		pbx_log(LOG_ERROR, "SCCP: (sccp_wrapper_asterisk116_setNativeVideoFormats) no channel provided!\n");
		return 0;
	}
	struct ast_format_cap *caps = ast_format_cap_alloc(AST_FORMAT_CAP_FLAG_DEFAULT);
	if (!caps) {
...
static boolean_t sccp_wrapper_asterisk116_setWriteFormat(constChannelPtr channel, skinny_codec_t codec)
{
	...
	//sccp_log((DEBUGCAT_RTP | DEBUGCAT_CODEC)) (VERBOSE_PREFIX_3 "%s: (asterisk116_setWriteFormat) %s)\n", channel->designator, codec2str(codec));

	struct ast_format *ast_format = sccp_asterisk116_skinny2ast_format(codec);
...
static boolean_t sccp_wrapper_asterisk116_setReadFormat(constChannelPtr channel, skinny_codec_t codec)
{
	...
	//sccp_log((DEBUGCAT_RTP | DEBUGCAT_CODEC)) (VERBOSE_PREFIX_3 "%s: (asterisk116_setReadFormat) %s)\n", channel->designator, codec2str(codec));
...
static void sccp_wrapper_asterisk116_setCalleridName(PBX_CHANNEL_TYPE *pbx_channel, const char *name)
{
	if (pbx_channel && name) {
...
static void sccp_wrapper_asterisk116_setCalleridNumber(PBX_CHANNEL_TYPE *pbx_channel, const char *number)
{
	if (pbx_channel && number) {
...
static void sccp_wrapper_asterisk116_setCalleridAni(PBX_CHANNEL_TYPE *pbx_channel, const char *number)
{
	if (pbx_channel && number) {
...
static void sccp_wrapper_asterisk116_setRedirectingParty(PBX_CHANNEL_TYPE *pbx_channel, const char *number, const char *name)
{
	if (pbx_channel) {
		if (number) {
...
static void sccp_wrapper_asterisk116_setRedirectedParty(PBX_CHANNEL_TYPE *pbx_channel, const char *number, const char *name)
{
	if (pbx_channel) {
		if (number) {
...
static void sccp_wrapper_asterisk116_updateConnectedLine(constChannelPtr channel, const char *number, const char *name, uint8_t reason)
{
	if (!channel || !channel->owner) {
		return;
	}
	__sccp_asterisk116_updateConnectedLine(channel->owner, number,name, reason);
}

static int sccp_wrapper_asterisk116_sched_add(int when, sccp_sched_cb callback, const void *data)
{
	if (sched) {
...
static int sccp_wrapper_asterisk116_sched_del(int id)
{
	if (sched) {
...
static int sccp_wrapper_asterisk116_sched_add_ref(int *id, int when, sccp_sched_cb callback, sccp_channel_t * channel)
{
	if (sched && channel) {
...
static int sccp_wrapper_asterisk116_sched_del_ref(int *id, sccp_channel_t *channel)
{
	if (sched) {
...
static int sccp_wrapper_asterisk116_sched_replace_ref(int *id, int when, ast_sched_cb callback, sccp_channel_t *channel)
{
	if (sched) {
...
static long sccp_wrapper_asterisk116_sched_when(int id)
{
	if (sched) {
...
static int sccp_wrapper_asterisk116_sched_wait(int id)
{
	if (sched) {
...
static int sccp_wrapper_asterisk116_setCallState(constChannelPtr channel, int state)
{
...
static PBX_CHANNEL_TYPE *sccp_wrapper_asterisk116_findChannelWithCallback(int (*const found_cb) (PBX_CHANNEL_TYPE * c, void *data), void *data, boolean_t lock)
{
	PBX_CHANNEL_TYPE *remotePeer = NULL;
...
#if 0
static int sccp_wrapper_asterisk116_setOption(PBX_CHANNEL_TYPE * ast, int option, void *data, int datalen)
{
	int res = -1;
	AUTO_RELEASE(sccp_channel_t, c , get_sccp_channel_from_pbx_channel(ast));
	...
	if (c) {
		...
		switch (option) {
			case AST_OPTION_FORMAT_READ:
				...
				//sccp_wrapper_asterisk116_setReadFormat(c, (struct ast_format *) data);
				break;
			case AST_OPTION_FORMAT_WRITE:
				if (c->rtp.audio.instance) {
					res = ast_rtp_instance_set_write_format(c->rtp.audio.instance, (struct ast_format *) data);
				}
				//sccp_wrapper_asterisk116_setWriteFormat(c, (struct ast_format *) data);
				break;
 ...
 */
static skinny_busylampfield_state_t sccp_wrapper_asterisk116_getExtensionState(const char *extension, const char *context)
{
...
static struct ast_endpoint *sccp_wrapper_asterisk116_endpoint_create(const char *tech, const char *resource)
{
	return ast_endpoint_create(tech, resource);
}

static void sccp_wrapper_asterisk116_endpoint_online(struct ast_endpoint *endpoint, const char *address)
{
...
static void sccp_wrapper_asterisk116_endpoint_offline(struct ast_endpoint *endpoint, const char *cause)
{
	RAII_VAR(struct ast_json *, blob, NULL, ast_json_unref);
...
static void sccp_wrapper_asterisk116_endpoint_shutdown(struct ast_endpoint **endpoint)
{
	ast_endpoint_shutdown(*endpoint);
	*endpoint = NULL;
}

static int sccp_wrapper_asterisk116_dumpchan(struct ast_channel *c, char *buf, size_t size)
{
...
static boolean_t sccp_wrapper_asterisk116_channelIsBridged(sccp_channel_t * channel)
{
	boolean_t res = FALSE;
...
static PBX_CHANNEL_TYPE *sccp_wrapper_asterisk116_getBridgeChannel(PBX_CHANNEL_TYPE * pbx_channel)
{
...
static boolean_t sccp_wrapper_asterisk116_attended_transfer(sccp_channel_t * destination_channel, sccp_channel_t * source_channel)
{
...
struct ast_rtp_glue sccp_rtp = {
	...
	type:	SCCP_TECHTYPE_STR,
	mod:	NULL,
	get_rtp_info:sccp_wrapper_asterisk116_get_rtp_info,
	//allow_rtp_remote:sccp_wrapper_asterisk116_allow_rtp_remote, 		/* check c->directmedia and return 1 if ok */
	get_vrtp_info:sccp_wrapper_asterisk116_get_vrtp_info,
	//allow_vrtp_remote:sccp_wrapper_asterisk116_allow_vrtp_remote, 	/* check c->directmedia and return 1 if ok */
	get_trtp_info:NULL,
	update_peer:sccp_wrapper_asterisk116_update_rtp_peer,
	get_codec:sccp_wrapper_asterisk116_getCodec,
	/* *INDENT-ON* */
};
#else
struct ast_rtp_glue sccp_rtp = {
...
static boolean_t sccp_wrapper_asterisk116_setLanguage(PBX_CHANNEL_TYPE * pbxChannel, const char *language)
{
...
const PbxInterface iPbx = {
	...
	alloc_pbxChannel:		sccp_wrapper_asterisk116_allocPBXChannel,
	set_callstate:			sccp_wrapper_asterisk116_setCallState,
	checkhangup:			sccp_wrapper_asterisk116_checkHangup,
	hangup:				NULL,
	extension_status:		sccp_wrapper_asterisk116_extensionStatus,

	setPBXChannelLinkedId:		sccp_wrapper_asterisk_set_pbxchannel_linkedid,
	/** get channel by name */
	getChannelByName:		sccp_wrapper_asterisk116_getChannelByName,
	...
	set_nativeAudioFormats:		sccp_wrapper_asterisk116_setNativeAudioFormats,
	set_nativeVideoFormats:		sccp_wrapper_asterisk116_setNativeVideoFormats,

	getPeerCodecCapabilities:	NULL,
	send_digit:			sccp_wrapper_sendDigit,
	send_digits:			sccp_wrapper_sendDigits,

	sched_add:			sccp_wrapper_asterisk116_sched_add,
	sched_del:			sccp_wrapper_asterisk116_sched_del,
	sched_add_ref:			sccp_wrapper_asterisk116_sched_add_ref,
	sched_del_ref:			sccp_wrapper_asterisk116_sched_del_ref,
	sched_replace_ref:		sccp_wrapper_asterisk116_sched_replace_ref,
	sched_when:			sccp_wrapper_asterisk116_sched_when,
	sched_wait:			sccp_wrapper_asterisk116_sched_wait,

	/* rtp */
	rtp_getPeer:			sccp_wrapper_asterisk116_rtpGetPeer,
	rtp_getUs:			sccp_wrapper_asterisk116_rtpGetUs,
	rtp_setPhoneAddress:		sccp_wrapper_asterisk116_setPhoneRTPAddress,
	rtp_setWriteFormat:		sccp_wrapper_asterisk116_setWriteFormat,
	rtp_setReadFormat:		sccp_wrapper_asterisk116_setReadFormat,
	rtp_destroy:			sccp_wrapper_asterisk116_destroyRTP,
	rtp_stop:			ast_rtp_instance_stop(rtp),
	rtp_codec:			NULL,
	rtp_create_instance:		sccp_wrapper_asterisk116_createRtpInstance,
	rtp_get_payloadType:		sccp_wrapper_asterisk116_get_payloadType,
	rtp_get_sampleRate:		sccp_wrapper_asterisk116_get_sampleRate,
	rtp_bridgePeers:		NULL,

	/* callerid */
	get_callerid_name:		sccp_wrapper_asterisk116_callerid_name,
	get_callerid_number:		sccp_wrapper_asterisk116_callerid_number,
	get_callerid_ton:		sccp_wrapper_asterisk116_callerid_ton,
	get_callerid_ani:		sccp_wrapper_asterisk116_callerid_ani,
	get_callerid_subaddr:		sccp_wrapper_asterisk116_callerid_subaddr,
	get_callerid_dnid:		sccp_wrapper_asterisk116_callerid_dnid,
	get_callerid_rdnis:		sccp_wrapper_asterisk116_callerid_rdnis,
	get_callerid_presentation:	sccp_wrapper_asterisk116_callerid_presentation,

	set_callerid_name:		sccp_wrapper_asterisk116_setCalleridName,
	set_callerid_number:		sccp_wrapper_asterisk116_setCalleridNumber,
	set_callerid_ani:		sccp_wrapper_asterisk116_setCalleridAni,
	set_callerid_dnid:		NULL,
	set_callerid_redirectingParty:	sccp_wrapper_asterisk116_setRedirectingParty,
	set_callerid_redirectedParty:	sccp_wrapper_asterisk116_setRedirectedParty,
	set_callerid_presentation:	sccp_wrapper_asterisk116_setCalleridPresentation,

	set_dialed_number:		sccp_wrapper_asterisk116_setDialedNumber,
	set_connected_line:		sccp_wrapper_asterisk116_updateConnectedLine,
	sendRedirectedUpdate:		sccp_asterisk_sendRedirectedUpdate,

	/* feature section */
	feature_park:			sccp_wrapper_asterisk116_park,
	...
	getFeatureExtension:		sccp_wrapper_asterisk116_getFeatureExtension,
	getPickupExtension:		sccp_wrapper_asterisk116_getPickupExtension,

	eventSubscribe:			NULL,
	findChannelByCallback:		sccp_wrapper_asterisk116_findChannelWithCallback,
	...
	allocTempPBXChannel:		sccp_wrapper_asterisk116_allocTempPBXChannel,
	masqueradeHelper:		sccp_wrapper_asterisk116_masqueradeHelper,
	requestAnnouncementChannel:	sccp_wrapper_asterisk116_requestAnnouncementChannel,

	set_language:			sccp_wrapper_asterisk116_setLanguage,

	getExtensionState:		sccp_wrapper_asterisk116_getExtensionState,
	findPickupChannelByExtenLocked:	sccp_wrapper_asterisk116_findPickupChannelByExtenLocked,
	findPickupChannelByGroupLocked:	sccp_wrapper_asterisk116_findPickupChannelByGroupLocked,

 	endpoint_create:		sccp_wrapper_asterisk116_endpoint_create,
	endpoint_online:		sccp_wrapper_asterisk116_endpoint_online,
	endpoint_offline:		sccp_wrapper_asterisk116_endpoint_offline,
	endpoint_shutdown:		sccp_wrapper_asterisk116_endpoint_shutdown,

	set_owner:			sccp_wrapper_asterisk116_setOwner,
	removeTimingFD:			sccp_wrapper_asterisk116_removeTimingFD,
	dumpchan:			sccp_wrapper_asterisk116_dumpchan,
	channel_is_bridged:		sccp_wrapper_asterisk116_channelIsBridged,
	get_bridged_channel:		sccp_wrapper_asterisk116_getBridgeChannel,
	get_underlying_channel:		sccp_wrapper_asterisk116_getBridgeChannel,
	attended_transfer:		sccp_wrapper_asterisk116_attended_transfer,
