...
static boolean_t sccp_wrapper_asterisk110_attended_transfer(sccp_channel_t * transferee, sccp_channel_t * transferer)
{
	boolean_t res = FALSE;
	...
	if (sccp_wrapper_asterisk110_channelIsBridged(transferer)) {
		bridged_channel = sccp_wrapper_asterisk110_getBridgeChannel(transferer_pbx_channel);
		destination_channel = transferee_pbx_channel;
	} else if (sccp_wrapper_asterisk110_channelIsBridged(transferee)) {					// one legged blind transfer
		two_legged_transfer = FALSE;
		bridged_channel = sccp_wrapper_asterisk110_getBridgeChannel(transferee_pbx_channel);
...
static boolean_t sccp_wrapper_asterisk110_blind_transfer(sccp_channel_t * transferee, const char *extension, const char *context)
{
	boolean_t res = FALSE;
	if (!transferee || !transferee->owner || !extension || !context) {
		return res;
	}
	PBX_CHANNEL_TYPE *transferee_bridged_channel = sccp_wrapper_asterisk110_getBridgeChannel(transferee->owner);
...
const PbxInterface iPbx = {
	...
	blind_transfer:			sccp_wrapper_asterisk110_blind_transfer,
