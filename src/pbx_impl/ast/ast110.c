...
//                              ast_log(LOG_WARNING, "%s: Asked to transmit frame type %d, while native formats are %s(%lu) read/write = %s(%lu)/%s(%lu)\n", DEV_ID_LOG(c->getDevice(c)), (int)frame->frametype, pbx_getformatname_multiple(s1, sizeof(s1) - 1, ast->nativeformats), ast->nativeformats, pbx_getformatname_multiple(s2, sizeof(s2) - 1, ast->readformat), (ULONG)ast->readformat, pbx_getformatname_multiple(s3, sizeof(s3) - 1, (ULONG)ast->writeformat), (ULONG)ast->writeformat);
				//return -1;
			}
			if (c->rtp.audio.rtp) {
...
static boolean_t sccp_wrapper_asterisk110_allocPBXChannel(const sccp_channel_t * channel, PBX_CHANNEL_TYPE ** pbx_channel)
{
...
static int sccp_wrapper_asterisk110_hangup(PBX_CHANNEL_TYPE * ast_channel)
{
	sccp_channel_t *c;
	int res;
...
static uint8_t sccp_wrapper_asterisk110_get_sampleRate(skinny_codec_t codec)
{
	struct ast_format astCodec;
...
//static int sccp_wrapper_asterisk110_set_rtp_peer(PBX_CHANNEL_TYPE *ast, PBX_RTP_TYPE *rtp, PBX_RTP_TYPE *vrtp, PBX_RTP_TYPE *trtp, int codecs, int nat_active)
// static int sccp_wrapper_asterisk110_set_rtp_peer(PBX_CHANNEL_TYPE * ast, PBX_RTP_TYPE * rtp, PBX_RTP_TYPE * vrtp, PBX_RTP_TYPE * trtp, format_t codecs, int nat_active)
...
static boolean_t sccp_wrapper_asterisk110_create_audio_rtp(const sccp_channel_t * c, void **new_rtp)
{
	sccp_session_t *s;
	sccp_device_t *d = NULL;
...
static boolean_t sccp_wrapper_asterisk110_create_video_rtp(const sccp_channel_t * c, void **new_rtp)
{
	sccp_session_t *s;
	sccp_device_t *d = NULL;
	struct ast_sockaddr sock;
...
static boolean_t sccp_wrapper_asterisk110_addToDatabase(const char *family, const char *key, const char *value)
{
	int res;
...
static boolean_t sccp_wrapper_asterisk110_getFromDatabase(const char *family, const char *key, char *out, int outlen)
{
	int res;
...
static boolean_t sccp_wrapper_asterisk110_removeFromDatabase(const char *family, const char *key)
{
	int res;
...
static boolean_t sccp_wrapper_asterisk110_removeTreeFromDatabase(const char *family, const char *key)
{
	int res;
...
static boolean_t sccp_wrapper_asterisk110checkhangup(const sccp_channel_t * channel)
{
	int res;
...
static unsigned int sccp_wrapper_asterisk110_sched_wait(int id)
{
...
static int sccp_wrapper_asterisk110_requestHangup(PBX_CHANNEL_TYPE * channel)
{
 ...
 */
static int sccp_asterisk_channel_read(struct ast_channel *ast, NEWCONST char *funcname, char *args, char *buf, size_t buflen)
{
	sccp_channel_t *c;
...
static int sccp_asterisk_channel_write(struct ast_channel *ast, const char *funcname, char *args, const char *value){
	sccp_channel_t *c;
	boolean_t	res;
...
struct ast_channel_tech sccp_tech = {
	...
	.func_channel_read 	= sccp_asterisk_channel_read,
	.func_channel_write 	= sccp_asterisk_channel_write,
...
struct sccp_pbx_cb sccp_pbx = {
	...
	.checkhangup			= sccp_wrapper_asterisk110checkhangup,
	...
	/* database */
	.feature_addToDatabase 		= sccp_wrapper_asterisk110_addToDatabase,
	.feature_getFromDatabase 	= sccp_wrapper_asterisk110_getFromDatabase,				//! \todo implement callback
	.feature_removeFromDatabase     = sccp_wrapper_asterisk110_removeFromDatabase,	
	.feature_removeTreeFromDatabase = sccp_wrapper_asterisk110_removeTreeFromDatabase,
