...
static int sccp_astwrap_indicate(PBX_CHANNEL_TYPE * ast, int ind, const void *data, size_t datalen)
{
	...
	sccp_log((DEBUGCAT_PBX | DEBUGCAT_CHANNEL | DEBUGCAT_INDICATE)) (VERBOSE_PREFIX_3 "%s: (pbx_indicate) start indicate '%s' (%d) condition on channel %s (readStat:%d, reception.state:%d, rtp:%s)\n", DEV_ID_LOG(d), asterisk_indication2str(ind), ind, pbx_channel_name(ast), c->rtp.audio.reception.state, c->rtp.audio.reception.state, (c->rtp.audio.instance) ? "yes" : "no");
...
*/
		...
		if (d->directrtp && d->nat < SCCP_NAT_ON && !nat_active && !c->conference) {			// assume directrtp
			...
			if(!sccp_netsock_is_any_addr(&sas)) {
				if(d->nat == SCCP_NAT_OFF) {                                        // forced nat off to circumvent autodetection + direcrtp, requires checking both phone_ip and external session ip address
												    // against devices permit/deny
		...
		sccp_log((DEBUGCAT_RTP))(VERBOSE_PREFIX_1
					 ...
					 S_COR(directmedia, "yes", "no"), S_COR(!c->conference, "yes", "no"));
