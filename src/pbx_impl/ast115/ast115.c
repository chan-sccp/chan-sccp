...
static uint8_t sccp_asterisk115_getSkinnyFormatMultiple(struct ast_format_cap *ast_format_capability, skinny_codec_t codecs[], int length)
{
		...
		if (pbx_find_channel_by_linkid(remotePeer, (void *) ast_channel_linkedid(ast))) {
			...
			if (ast_format_cap_count(ast_channel_nativeformats(remotePeer)) > 0) {
				...
				if (joint) {
					...
					if (!ast_format_cap_get_compatible(caps, ast_channel_nativeformats(remotePeer), joint)) {
						best_fmt_native = ast_format_cap_get_best_by_type(joint, AST_MEDIA_TYPE_AUDIO);
					} else {
						ast_translator_best_choice(caps, joint, &best_fmt_cap, &best_fmt_native);
					}
			...
			sccp_asterisk115_getSkinnyFormatMultiple(ast_channel_nativeformats(c->owner), c->preferences.audio, ARRAY_LEN(c->preferences.audio));
...
static int sccp_wrapper_asterisk115_indicate(PBX_CHANNEL_TYPE * ast, int ind, const void *data, size_t datalen)
{
	...
	sccp_log((DEBUGCAT_PBX | DEBUGCAT_CHANNEL | DEBUGCAT_INDICATE)) (VERBOSE_PREFIX_1
		"%s: (pbx_indicate) start indicate '%s' (%d) condition on channel %s (rtp_instance:%s, receiveChannelState:%d/%s, mediaTransmissionState:%d/%s)\n", 
		DEV_ID_LOG(d), asterisk_indication2str(ind), ind, pbx_channel_name(ast), 
		c->rtp.audio.instance ? "yes" : "no",
		c->rtp.audio.receiveChannelState, codec2str(c->rtp.audio.readFormat), c->rtp.audio.mediaTransmissionState, codec2str(c->rtp.audio.writeFormat)
	);
...
static int sccp_wrapper_asterisk115_setNativeAudioFormats(constChannelPtr channel, skinny_codec_t codecs[], int length)
{
...
static boolean_t sccp_wrapper_asterisk115_setWriteFormat(constChannelPtr channel, skinny_codec_t codec)
{
	...
	sccp_log((DEBUGCAT_RTP | DEBUGCAT_CODEC)) (VERBOSE_PREFIX_3 "%s: (asterisk115_setWriteFormat) %s)\n", channel->designator, codec2str(codec));
...
static boolean_t sccp_wrapper_asterisk115_setReadFormat(constChannelPtr channel, skinny_codec_t codec)
{
	...
	sccp_log((DEBUGCAT_RTP | DEBUGCAT_CODEC)) (VERBOSE_PREFIX_3 "%s: (asterisk115_setReadFormat) %s)\n", channel->designator, codec2str(codec));
	...
	if (requestor) {
		...
		if (remoteSccpChannel) {
			...
			if (acaps) {
				...
				audio_codec = sccp_asterisk115_getSkinnyFormatSingle(acaps);
				if (!sccp_asterisk115_getSkinnyFormatMultiple(acaps, audioCapabilities, ARRAY_LEN(audioCapabilities))) {
			...
			if (vcaps) {
				...
				video_codec = sccp_asterisk115_getSkinnyFormatSingle(vcaps);
				if (!sccp_asterisk115_getSkinnyFormatMultiple(vcaps, videoCapabilities, ARRAY_LEN(videoCapabilities))) {
					pbx_log(LOG_NOTICE, "SCCP: remote native format is not compatible with any skinny format. Transcoding required\n");
...
static int sccp_wrapper_asterisk115_answer(PBX_CHANNEL_TYPE * pbxchan)
{
