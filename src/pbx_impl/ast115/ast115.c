...
static boolean_t sccp_wrapper_asterisk115_createRtpInstance(constDevicePtr d, constChannelPtr c, sccp_rtp_t *rtp)
{
	...
	//if (rtp->type == SCCP_RTP_AUDIO && SCCP_DTMFMODE_SKINNY == d->dtmfmode) {
		...
		//ast_rtp_codecs_payloads_set_rtpmap_type(ast_rtp_instance_get_codecs(instance), instance, 121, "audio", "cisco-telephone-event", 0);
		//ast_rtp_codecs_payload_code_tx(ast_rtp_instance_get_codecs(instance), 0, NULL, AST_RTP_DTMF)
	//}
	ast_rtp_codecs_set_framing(ast_rtp_instance_get_codecs(instance), ast_format_get_default_ms(ast_format_cap_get_format(ast_channel_nativeformats(c->owner), 0)));
...
static int sccp_wrapper_asterisk115_setNativeAudioFormats(constChannelPtr channel, skinny_codec_t codecs[], int length)
{
	if (!channel || !channel->owner || !ast_channel_nativeformats(channel->owner)) {
		pbx_log(LOG_ERROR, "SCCP: (sccp_wrapper_asterisk115_setNativeAudioFormats) no channel provided!\n");
		return 0;
	}
	int n;
...
{
	if (!channel || !channel->owner || !ast_channel_nativeformats(channel->owner)) {
		pbx_log(LOG_ERROR, "SCCP: (sccp_wrapper_asterisk115_setNativeVideoFormats) no channel provided!\n");
		return 0;
	}
	//int n = 0;
	//int length = 1;
