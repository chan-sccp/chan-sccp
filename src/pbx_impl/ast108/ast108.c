	...
	if (c) {
		if (c->owner != oldchan) {
			...
			if (!strstr(pbx_channel_name(newchan), "<ZOMBIE>")) {
				...
				// c->hangupRequest = sccp_astgenwrap_dummyHangup;
...
static int sccp_astwrap_update_rtp_peer(PBX_CHANNEL_TYPE * ast, PBX_RTP_TYPE * rtp, PBX_RTP_TYPE * vrtp, PBX_RTP_TYPE * trtp, format_t codecs, int nat_active)
{
	//AUTO_RELEASE(sccp_channel_t, c , get_sccp_channel_from_pbx_channel(ast));				// not following the refcount rules... channel is already retained
	sccp_channel_t *c = NULL;
	int result = 0;
...
static format_t sccp_astwrap_getCodec(PBX_CHANNEL_TYPE * ast)
{
...
static uint sccp_wrapper_get_dtmf_payload_code(constChannelPtr c)
{
	...
	if (SCCP_DTMFMODE_SKINNY != c->dtmfmode) {
		rtp_code = ast_rtp_codecs_payload_code(ast_rtp_instance_get_codecs(c->rtp.audio.instance), 0, AST_RTP_DTMF);
	}
...
static void sccp_astgenwrap_set_pbxchannel_linkedid(PBX_CHANNEL_TYPE * pbx_channel, const char *new_linkedid)
{
	if (pbx_channel) {
...
static const char *sccp_astgenwrap_get_channel_##_field(const sccp_channel_t * channel)	 		\
{														\
...
static void sccp_astgenwrap_set_channel_##_field(const sccp_channel_t * channel, const char * (_field))	\
{ 														\
	if (channel && channel->owner) {											\
...
static void sccp_astgenwrap_set_channel_linkedid(const sccp_channel_t * channel, const char *new_linkedid)
{
	if (channel && channel->owner) {
		sccp_astgenwrap_set_pbxchannel_linkedid(channel->owner, new_linkedid);
	}
}

static void sccp_astgenwrap_set_channel_call_forward(const sccp_channel_t * channel, const char *new_call_forward)
{
	if (channel && channel->owner) {
...
static void sccp_astgenwrap_set_channel_name(const sccp_channel_t * channel, const char *new_name)
{
	if (channel && channel->owner) {
...
static enum ast_channel_state sccp_astgenwrap_get_channel_state(const sccp_channel_t * channel)
{
	if (channel && channel->owner) {
...
static const struct ast_pbx *sccp_astgenwrap_get_channel_pbx(const sccp_channel_t * channel)
{
	if (channel && channel->owner) {
...
static boolean_t sccp_astgenwrap_setLanguage(PBX_CHANNEL_TYPE * pbxChannel, const char *newLanguage)
{
...
const PbxInterface iPbx = {
	...
	setPBXChannelLinkedId:		sccp_astgenwrap_set_pbxchannel_linkedid,
	/** get channel by name */
	getChannelByName:		sccp_astwrap_getChannelByName,
	getRemoteChannel:		sccp_astwrap_getRemoteChannel,
	getChannelByCallback:		NULL,

	getChannelLinkedId:		sccp_astgenwrap_get_channel_linkedid,
	setChannelLinkedId:		sccp_astgenwrap_set_channel_linkedid,
	getChannelName:			sccp_astgenwrap_get_channel_name,
	setChannelName:			sccp_astgenwrap_set_channel_name,
	getChannelUniqueID:		sccp_astgenwrap_get_channel_uniqueid,
	getChannelExten:		sccp_astgenwrap_get_channel_exten,
	setChannelExten:		sccp_astgenwrap_set_channel_exten,
	getChannelContext:		sccp_astgenwrap_get_channel_context,
	setChannelContext:		sccp_astgenwrap_set_channel_context,
	getChannelMacroExten:		sccp_astgenwrap_get_channel_macroexten,
	setChannelMacroExten:		sccp_astgenwrap_set_channel_macroexten,
	getChannelMacroContext:		sccp_astgenwrap_get_channel_macrocontext,
	setChannelMacroContext:		sccp_astgenwrap_set_channel_macrocontext,
	getChannelCallForward:		sccp_astgenwrap_get_channel_call_forward,
	setChannelCallForward:		sccp_astgenwrap_set_channel_call_forward,

	getChannelAppl:			sccp_astgenwrap_get_channel_appl,
	getChannelState:		sccp_astgenwrap_get_channel_state,
	getChannelPbx:			sccp_astgenwrap_get_channel_pbx,
	...
	/* callerid */
	get_callerid_pname:		sccp_astwrap_callerid_name,
	...
	set_language:			sccp_astgenwrap_setLanguage,
