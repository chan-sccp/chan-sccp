...
static uint8_t sccp_asterisk113_getSkinnyFormatMultiple(struct ast_format_cap *ast_format_capability, skinny_codec_t codecs[], int length)
{
...
static struct ast_format *sccp_asterisk113_skinny2ast_format(skinny_codec_t skinnycodec)
{
	switch (skinnycodec) {
...
static void pbx_format_cap_append_skinny(struct ast_format_cap *caps, skinny_codec_t codecs[SKINNY_MAX_CAPABILITIES]) {
	...
	for (i=0; i<SKINNY_MAX_CAPABILITIES; i++) {
		...
		struct ast_format *format = sccp_asterisk113_skinny2ast_format(codecs[i]);
		...
		case AST_CONTROL_RINGING:
			if (SKINNY_CALLTYPE_OUTBOUND == c->calltype) {
				if (c->remoteCapabilities.audio[0] == SKINNY_CODEC_NONE) {
					...
					/* fixate native Audio Formats */
					//skinny_codec_t joint = SKINNY_CODEC_NONE;
					//joint = sccp_codec_findBestJoint(c, c->preferences.audio, c->remoteCapabilities.audio);
					//skinny_codec_t codecs[SKINNY_MAX_CAPABILITIES] = { joint, 0};
					//iPbx.set_nativeAudioFormats(c, codecs, 1);
				}
	...
	if (requestor) {
		...
		if (remoteSccpChannel) {
			...
			if (acaps) {
				...
				audio_codec = sccp_asterisk113_getSkinnyFormatSingle(acaps);
				if (!sccp_asterisk113_getSkinnyFormatMultiple(acaps, audioCapabilities, ARRAY_LEN(audioCapabilities))) {
			...
			if (vcaps) {
				...
				video_codec = sccp_asterisk113_getSkinnyFormatSingle(vcaps);
				if (!sccp_asterisk113_getSkinnyFormatMultiple(vcaps, videoCapabilities, ARRAY_LEN(videoCapabilities))) {
					pbx_log(LOG_NOTICE, "SCCP: remote native format is not compatible with any skinny format. Transcoding required\n");
...
static void sccp_wrapper_asterisk113_getCodec(PBX_CHANNEL_TYPE * ast, struct ast_format_cap *result)
{
	...
	for (i = 0; i < ARRAY_LEN(channel->preferences.audio); i++) {
		struct ast_format *ast_format = sccp_asterisk113_skinny2ast_format(channel->preferences.audio[i]);
		if (ast_format != ast_format_none) {
			unsigned int framing = ast_format_get_default_ms(ast_format);
	...
	for (i = 0; i < ARRAY_LEN(channel->preferences.video); i++) {
		struct ast_format *ast_format = sccp_asterisk113_skinny2ast_format(channel->preferences.video[i]);
		if (ast_format != ast_format_none) {
			unsigned int framing = ast_format_get_default_ms(ast_format);
			ast_format_cap_append(result, ast_format, framing);
		}
