...
static sccp_moo_t *sccp_process_data(sccp_session_t * s)
{
	...
	if (newPacketSize > SCCP_MAX_PACKET) {									/* Make sure we don't read bigger then SCCP_MAX_PACKET */
		pbx_log(LOG_NOTICE, "SCCP: Oversize packet mid: %d, our packet size: %d, phone packet size: %d, max packat size: %lu (Expect Problems. Report to Developers)\n", messageId, newPacketSize, packetSize, SCCP_MAX_PACKET);
