...
void __sccp_indicate(sccp_device_t * device, sccp_channel_t * c, sccp_channelstate_t state, uint8_t debug, char *file, int line, const char *pretty_function)
{
	...
	switch (state) {
		...
		case SCCP_CHANNELSTATE_RINGING:
			...
			snprintf(prompt, sizeof(prompt), "%s" SKINNY_DISP_FROM ": %s", (c->ringermode == SKINNY_RINGTYPE_URGENT) ? SKINNY_DISP_FLASH : "", strlen(c->callInfo.callingPartyName) ? c->callInfo.callingPartyName : c->callInfo.callingPartyNumber);
		...
		case SCCP_CHANNELSTATE_CALLTRANSFER:
			...
			d->protocol->sendCallInfo(d, c, instance);
			break;
		case SCCP_CHANNELSTATE_HELP_FOR_TRANSFER:								/* \todo SCCP_CHANNELSTATE_BLINDTRANSFER To be implemented */
