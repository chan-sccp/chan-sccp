...
#else
/*!
 * \brief CLI Enable Filtered Debug
 * \return Asterisk Cli Entry Structure
 * \note Alias for Asterisk CLI Entry
 */
...
static char *cli_show_mwi_subscriptions(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a) {
        ...
        if (sccp_cli_do_show_mwi_subscriptions(a->fd, a->argc, a->argv) == RESULT_SUCCESS)
				return CLI_SUCCESS;
		else
				return CLI_FAILURE;
...
static struct ast_cli_entry cli_show_mwi_subscriptions = {
        ...
        sccp_cli_do_show_mwi_subscriptions,
        "SCCP show subscriptions",
        "Usage: SCCP sccp show subscriptions\n"
        "		Show the SCCP channel subscriptions\n"
};
#endif
...
/**
 * \brief Register CLI functions from Asterisk
 */
...
/**
 * \brief Unregister CLI functions from Asterisk
 */
