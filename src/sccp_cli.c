...
//	CLI_AMI_OUTPUT_PARAM("Extern IP", CLI_AMI_LIST_WIDTH, "%s", sccp_socket_stringify(&GLOB(externip)) );		// deprecated
	...
	CLI_AMI_OUTPUT_PARAM("Early RTP", CLI_AMI_LIST_WIDTH, "%s (%s)", GLOB(earlyrtp) ? "Yes" : "No", GLOB(earlyrtp) ? sccp_earlyrtp2str(GLOB(earlyrtp)) : "none");
...
static int sccp_show_device(int fd, int *total, struct mansession *s, const struct message *m, int argc, char *argv[])
{
	...
	char binstr[32+1];
	CLI_AMI_OUTPUT_PARAM("Device Features",		CLI_AMI_LIST_WIDTH, "%#1x, 0b%s", d->device_features, sccp_bin2str(binstr, sizeof(binstr), d->device_features));
...
//	CLI_AMI_OUTPUT_PARAM("digit timeout",		CLI_AMI_LIST_WIDTH, "%d", d->digittimeout);
	...
	CLI_AMI_OUTPUT_PARAM("Early RTP",		CLI_AMI_LIST_WIDTH, "%s (%s)", d->earlyrtp ? "Yes" : "No", d->earlyrtp ? sccp_earlyrtp2str(d->earlyrtp) : "none");
...
/*		CLI_AMI_TABLE_FIELD(DeviceDescr,	"-32.32s",	32,	d ? d->description : "(unknown)")		*/	\
		...
		CLI_AMI_TABLE_FIELD(SCCP State,		"-10.10s",	10,	sccp_channelstate2str(channel->state))			\
