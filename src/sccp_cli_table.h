...
#define UNIQUE_VAR(_a,_b) MERGE2_(_a,_b)
	...
	if (!s) {
		pbx_cli(fd,"+--- %s %.*s+\n",
			...
			UNIQUE_VAR(table_width_,CLI_AMI_TABLE_NAME) - (int)strlen(STRINGIFY(CLI_AMI_TABLE_NAME)) - 4,
			"------------------------------------------------------------------------------------------------------------------------------------------------------------------"
		);
		...
		astman_append(s, "TableName: %s\r\n",STRINGIFY(CLI_AMI_TABLE_NAME));local_total++;
		...
		if (!pbx_strlen_zero(UNIQUE_VAR(id, CLI_AMI_TABLE_NAME))) {
			...
			astman_append(s, "%s\r\n", UNIQUE_VAR(idtext,CLI_AMI_TABLE_NAME));local_total++;
		}
	}
	...
	if (!s) {
		...
		#endif
			CLI_AMI_TABLE_BEFORE_ITERATION
			astman_append(s, "Event: %sEntry\r\n",STRINGIFY(CLI_AMI_TABLE_PER_ENTRY_NAME));local_total++;
