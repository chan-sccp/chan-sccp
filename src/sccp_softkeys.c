...
{
	sccp_log((DEBUGCAT_SOFTKEY))(VERBOSE_PREFIX_3 "%s: SoftKey Park Pressed on line:%s\n", DEV_ID_LOG(d), l ? l->name : "<null>");
#ifdef CS_SCCP_PARK
	// if (c) {
	//	sccp_channel_park(c);
	//} else {
	iParkingLot.handleSoftkeyPress(d, l, lineInstance, c);
	//}
#else
...
static const struct sccp_softkeyMap_cb softkeyCbMap[] = {
	...
	{ SKINNY_LBL_PARK, FALSE, sccp_sk_park, NULL },
