...
/*typedef struct sccp_callinfo_data		sccp_callinfo_data_t;*/			/* SCCP Call Information Structure */
/*typedef struct sccp_update			sccp_update_t;*/			/* SCCP Update Structure */
...
typedef enum {
        FALSE					=0,
        TRUE					=1
} boolean_t;										/*!< ENUM for Boolean */

/*!
 * \brief Configuration status for global, line and device
 */
typedef enum {
        SCCP_CONFIG_STATUS_NOCHANGE		= 0,
        SCCP_CONFIG_STATUS_INTERNALCHANGE	= 1 << 1,
	SCCP_CONFIG_STATUS_PENDINGALCHANGE	= 1 << 2,
	SCCP_CONFIG_STATUS_OBSOLETE		= 1 << 3
} sccp_config_status_t;									/*!< Config Status */
...
typedef enum {
        ...
        FEATURE,
        EMPTY
} button_type_t;									/*!< Button Type */

/*!
 * \brief Call Answer Order Enum
 */
typedef enum {
        ANSWER_LAST_FIRST			=1,
        ANSWER_OLDEST_FIRST			=2
} call_answer_order_t;									/*!< Call Answer Order */

/*!
 * \brief MWI/LAMP/Light Enum
 * \todo One name for MWI, Lamp or Light
 */
typedef enum {
        ON,
        OFF
} light_t;										/*!< Light/MWI Status*/
...
};											/*!< SCCP SpeedDial Button Structure */
...
/*!
 * template structure to configure device defaults
 */
sccp_device_t *sccp_deviceTemplate;

/*!
 * template structure to configure line defaults
 */
