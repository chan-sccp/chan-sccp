...
void sccp_hint_module_start()
{
	...
	sccp_event_subscribe(SCCP_EVENT_LINESTATUS_CHANGED, sccp_hint_lineStateEventListener, TRUE);
...
void sccp_hint_module_stop()
{
	...
	sccp_event_unsubscribe(SCCP_EVENT_LINESTATUS_CHANGED, sccp_hint_lineStateEventListener);
}
...
/*!
 * \brief Event Listener for line state changes
 * \param event SCCP Event
 * 
 * \lock
 * 	- device
...
// 				}
		...
		sccp_log(DEBUGCAT_HINT) (VERBOSE_PREFIX_4 "partyName: %s\n", lineState->callInfo.partyName);
		sccp_log(DEBUGCAT_HINT) (VERBOSE_PREFIX_4 "partyNumber: %s\n", lineState->callInfo.partyNumber);
				...
				SCCP_LIST_TRAVERSE(&d->buttonconfig, buttonconfig, list) {
					if (buttonconfig->type == LINE) {
						...
						if (line) {
							...
							sccp_hint_lineStatusChanged(line, d, 0);
							line = sccp_line_release(line);
						}
					}
				}
