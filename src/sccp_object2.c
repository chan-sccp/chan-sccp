 ...
 * \note	SCCP Object Reference Container derived from Asterisk 1.6.1 "astobj2.c"
...
struct __priv_data
{
    ast_mutex_t lock;
    int ref_counter;
    so2_destructor_fn destructor_fn;
    /*! for stats */
    size_t data_size;
...
#define	SO2_MAGIC	0xa690b321

/*!
 * What an sccpobj2 object looks like: fixed-size private data
 ...
 *
 * \return the pointer to the sccpobj2 structure
 */
static inline struct sccpobj2 *INTERNAL_OBJ(void *user_data)
{
    struct sccpobj2 *p;
...
/*!
 * \brief convert from a pointer _p to an sccpobj2 object
 *
...
#else
int _so2_lock(void *user_data, const char *file, const char *func, int line, const char *var)
#endif
{
...
#ifdef SO2_DEBUG
    ast_atomic_fetchadd_int(&so2.total_locked, 1);
#endif
...
#ifndef DEBUG_THREADS
int so2_unlock(void *user_data)
#else
int _so2_unlock(void *user_data, const char *file, const char *func, int line, const char *var)
#endif
{
    struct sccpobj2 *p = INTERNAL_OBJ(user_data);
...
#ifndef DEBUG_THREADS
int so2_trylock(void *user_data)
#else
int _so2_trylock(void *user_data, const char *file, const char *func, int line, const char *var)
#endif
{
    struct sccpobj2 *p = INTERNAL_OBJ(user_data);
    int ret;
...
{
    struct sccpobj2 *p = INTERNAL_OBJ(obj);
...
int _so2_ref(void *user_data, const int delta)
{
    struct sccpobj2 *obj = INTERNAL_OBJ(user_data);
...
#ifdef SO2_DEBUG
    ast_atomic_fetchadd_int(&so2.total_refs, delta);
#endif
 ...
 */
static void *__so2_alloc(size_t data_size, so2_destructor_fn destructor_fn, const char *file, int line, const char *funcname)
{
    /* allocation */
    struct sccpobj2 *obj;
...
#ifdef SO2_DEBUG
    ast_atomic_fetchadd_int(&so2.total_objects, 1);
    ast_atomic_fetchadd_int(&so2.total_mem, data_size);
    ast_atomic_fetchadd_int(&so2.total_refs, 1);
#endif
...
void *_so2_alloc(size_t data_size, so2_destructor_fn destructor_fn)
{
...
#ifdef SO2_DEBUG
    ast_atomic_fetchadd_int(&so2.total_containers, 1);
#endif
...
static struct bucket_list *__so2_link(struct so2_container *c, void *user_data)
{
    ...
    /* the last two operations (so2_ref, so2_unlock) must be done by the calling func */
    return p;
}

void *_so2_link_debug(struct so2_container *c, void *user_data, char *tag, char *file, int line, const char *funcname)
{
    struct bucket_list *p = __so2_link(c, user_data);

    if (p)
    {
        _so2_ref_debug(user_data, +1, tag, file, line, funcname);
        so2_unlock(c);
    }
    return p;
}

void *_so2_link(struct so2_container *c, void *user_data)
{
    struct bucket_list *p = __so2_link(c, user_data);

    if (p)
    {
        _so2_ref(user_data, +1);
        so2_unlock(c);
    }
    return p;
}
 ...
 */
int so2_match_by_addr(void *user_data, void *arg, int flags)
{
    ...
    for (; i < last ; i++)
    {
        ...
        AST_LIST_TRAVERSE_SAFE_BEGIN(&c->buckets[i], cur, entry)
        {
            ...
            if (!(flags & OBJ_NODATA))  	/* if must return the object, record the value */
            {
                ...
                if (tag)
                    _so2_ref_debug(ret, 1, tag, file, line, funcname);
                else
                    _so2_ref(ret, 1);
            }
...
{
    return _so2_callback_debug(c, flags, c->cmp_fn, arg, tag, file, line, funcname);
}

void *_so2_find(struct so2_container *c, void *arg, enum search_flags flags)
{
    return _so2_callback(c, flags, c->cmp_fn, arg);
}
...
static void * __so2_iterator_next(struct so2_iterator *a, struct bucket_list **q)
{
    ...
    *q = NULL;
    ...
    if (!(a->flags & SO2_ITERATOR_DONTLOCK))
        so2_lock(a->c);
 ...
 * Print stats
 */
static char *handle_sccpobj2_stats(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
    switch (cmd)
    {
    case CLI_INIT:
...
static char *handle_sccpobj2_test(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
    ...
    if (prof_id == -1)
        prof_id = ast_add_profile("so2_alloc", 0);
     ...
     * so2_alloc() gives us a reference which we pass to the
...
static struct ast_cli_entry cli_sccpobj2[] =
{
...
{
#ifdef SO2_DEBUG
    ast_cli_register_multiple(cli_sccpobj2, ARRAY_LEN(cli_sccpobj2));
#endif
