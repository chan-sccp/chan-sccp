 ...
 * \return SCCP Device with default/global values
 */
...
sccp_device_t *sccp_device_applyDefaults(sccp_device_t *d)
{
        if (!d)
                return NULL;
        memcpy(&d, &sccp_deviceTemplate, sizeof(sccp_device_t));
        return d;
}
...
/*!
 * \brief Get Forward Status from Line/Device
 * \param l SCCP Line
 * \param device SCCP Device
 */
...
/*!
 * \brief Send Date/Time to Device
 * \param device SCCP Device
 */
