...
static int sccp_pbx_hangup(struct ast_channel * ast) {
	...
	while (c && sccp_mutex_trylock(&c->lock)) {
...
OUT:
	...
	if (d && d->session) {
		sccp_mutex_lock(&d->session->lock);
		d->session->needcheckringback = 1;
		sccp_mutex_unlock(&d->session->lock);
	}
...
uint8_t sccp_pbx_channel_allocate(sccp_channel_t * c) {
    ...
    tmp = ast_channel_alloc(1, AST_STATE_DOWN, l->cid_num, l->cid_name, l->accountcode, c->owner->exten, l->context, l->amaflags, "SCCP/%s", l->name);
#endif
