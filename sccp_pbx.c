...
/* this is for incoming calls asterisk sccp_request */
...
#ifdef CS_AST_HAS_AST_STRING_FIELD
		ast_string_field_set(tmp, musicclass, l->musicclass);
#else
...
dial:
	...
	//set private var
	if(chan){
		sccp_log(1)(VERBOSE_PREFIX_3 "%s: SET Var SKINNY_PRIVATE to: %s\n",DEV_ID_LOG(l->device), c->private ? "1" : "0");
