...
CREATE TABLE IF NOT EXISTS `sccpline` (
  ...
  `directed_pickup` ENUM('on','true','yes','off','false','no') default 'yes',
  `directed_pickup_context` varchar(100) default NULL,
  `pickup_modeanswer` ENUM('on','true','yes','off','false','no') default 'yes',
  `amaflags` varchar(45) default NULL,
  `dnd` ENUM('off','reject','silent','user') default 'reject',
  `videomode`  ENUM('user','auto','off') default 'auto',
